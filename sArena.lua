...
LSM:Register("statusbar", "Blizzard RetailBar", [[Interface\AddOns\sArena_Updated2_by_sammers\Textures\BlizzardRetailBar]])
...
--[[
    ImportOtherForkSettings: Migrates settings from other sArena versions to sArena Updated 2 by sammers
    
    This function handles the import process when users have multiple sArena versions installed
    and want to migrate their existing settings. It searches for saved
    variables from other sArena versions (including sArena_Reloaded), copies the data, and handles the addon switching.
]]
function sArenaMixin:ImportOtherForkSettings()
    -- Try to find the saved variables database from other sArena versions
    local oldDB = sArena_ReloadedDB or sArena2DB or sArenaDB or sArena_MoPDB
    ...
    -- Get reference to sArena Updated 2 by sammers's database
    local newDB = sArena3DB
...
local function TEMPShareCollectedData()
    if not sArena3DB or not sArena3DB.collectData then
        sArenaMixin:Print(L["DataCollection_NotEnabled"])
        return
    end

    local hasSpells = sArena3DB.collectedSpells and next(sArena3DB.collectedSpells) ~= nil
    local hasAuras = sArena3DB.collectedAuras and next(sArena3DB.collectedAuras) ~= nil
...
function sArenaMixin:OnEvent(event, ...)
            ...
            if noEarlyFrames then
                ...
                if sArena3DB.collectData then
                    ...
                    if combatEvent == "SPELL_CAST_SUCCESS" then
                        if not sArena3DB.collectedSpells then
                            sArena3DB.collectedSpells = {}
                        end
                        if not sArena3DB.collectedSpells[spellID] then
                            sArena3DB.collectedSpells[spellID] = {spellName, sourceClass, "CAST"}
                        end
                    elseif combatEvent == "SPELL_AURA_APPLIED" then
                        if not sArena3DB.collectedAuras then
                            sArena3DB.collectedAuras = {}
                        end
                        if not sArena3DB.collectedAuras[spellID] then
                            sArena3DB.collectedAuras[spellID] = {spellName, sourceClass, auraType}
                        end
                    end
                end
                -- TEMP DATACOLLECT
    ...
    elseif (event == "PLAYER_LOGIN") then
        ...
        --self:SetupCustomCD()
        if sArena3DB.reOpenOptions then
            sArena3DB.reOpenOptions = nil
...
local function ChatCommand(input)
    ...
    elseif cmd == "ver" or cmd == "version" then
        sArenaMixin:Print(L["Print_CurrentVersion"], C_AddOns.GetAddOnMetadata("sArena_Updated2_by_sammers", "Version"))
...
function sArenaMixin:UpdateNoTrinketTexture()
    if self.db.profile.removeUnequippedTrinketTexture then
        ...
        sArenaMixin.noTrinketTexture = "Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\inv_pet_exitbattle.tga"
    end
end
...
function sArenaFrameMixin:OnLoad()
    ...
    self.WidgetOverlay.focusIndicator.Texture:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\Waypoint-MapPin-Untracked.tga")
    self.WidgetOverlay.combatIndicator.Texture:SetAtlas("Food")
    self.WidgetOverlay.partyTarget1.Texture:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\GM-icon-headCount.tga")
    self.WidgetOverlay.partyTarget1.Texture:SetDesaturated(true)
    self.WidgetOverlay.partyTarget2.Texture:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\GM-icon-headCount.tga")
...
function sArenaFrameMixin:ResetLayout()
    ...
    self.AuraStacks:SetFont("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\arialn.ttf", 13, "THICKOUTLINE")
    self.DispelStacks:SetPoint("BOTTOMLEFT", self.Dispel.Texture, "BOTTOMLEFT", 2, 0)
    self.DispelStacks:SetFont("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\arialn.ttf", 15, "THICKOUTLINE")
            ...
            if drsEnabled > 0 then
                if not frame.fakeDRFrames then
                    ...
                    for drIndex = 1, 4 do
                        ...
                        fakeDRFrame.DRText:SetFont("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\arialn.ttf", 14, "OUTLINE")
            ...
            if modernCastbars then
                if keepDefaultModernTextures then
                    if isRetail then
                        ...
                        frame.CastBar:SetStatusBarTexture(data.unint and "Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-CastingBar-Uninterruptable" or data.channel and "Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-CastingBar-Filling-Channel" or "Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-CastingBar-Filling-Standard")
                    end
    ...
    if sArenaMixin.modernCastbars then
        ...
        elseif colors.enabled then
            if sArenaMixin.isMoP then
                castBar:SetStatusBarTexture(castBar.barType == "uninterruptable" and "Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-CastingBar-Uninterruptable" or castBar.barType == "channel" and "Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-CastingBar-Filling-Channel" or "Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-CastingBar-Filling-Standard")
            end
    ...
    if isMidnight then
        for i = 1, sArenaMixin.maxArenaOpponents do
            ...
            if useModern then
                ...
                newBar.MaskTexture:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\RetailCastMask.tga",
