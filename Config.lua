...
function sArenaMixin:GetLayoutOptionsTable(layoutName)
    ...
    optionsTable.widgets = {
        ...
        args = {
            ...
            focusIndicator = {
                order = 3,
                name = L["Widget_FocusIndicator"] .. " |TInterface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\Waypoint-MapPin-Untracked.tga:23:23|t",
                type = "group",
                inline = true,
                args = {
                    enabled = {
            ...
            partyTargetIndicators = {
                order = 4,
                name = L["Widget_PartyTargetIndicators"] .. " |TInterface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\GM-icon-headCount.tga:19:19|t",
                type = "group",
                inline = true,
                args = {
                    enabled = {
    ...
    for i = 1, sArenaMixin.maxArenaOpponents do
        ...
        if db.useModernCastbars then
            if isRetail then
                ...
                frame.CastBar.BorderShield:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-CastingBar-Shield.tga")
        ...
        for drIndex, drFrame in ipairs(drFrames) do
            if drFrame and drFrame.Icon then
                ...
                if not drFrame.DRTextFrame then
                    ...
                    drFrame.DRText:SetFont("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\arialn.ttf", 14, "OUTLINE")
                    ...
                    drFrame.DRText2:SetFont("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\arialn.ttf", 14, "OUTLINE")
                ...
                for drIndex, drFrame in ipairs(frame.drFrames) do
                    if drFrame then
                        ...
                        elseif db.drBorderGlowOff then
                            ...
                            if drFrame.Cooldown then
                                drFrame.Cooldown:SetSwipeTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\squarecutcornermask")
                            end
                            drFrame.Mask:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\squarecutcornermask", "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
                            if drFrame.Icon then
                        ...
                        elseif db.brightDRBorder then
                            ...
                            if drFrame.Border then
                                drFrame.Border:Show()
                                drFrame.Border:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-HUD-ActionBar-PetAutoCast-Mask.tga")
                            ...
                            if drFrame.BorderImmune then
                                drFrame.BorderImmune:Show()
                                drFrame.BorderImmune:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-HUD-ActionBar-PetAutoCast-Mask.tga")
                            end
                            if drFrame.PixelBorder then
                                drFrame.PixelBorder:Hide()
                            end
                            ...
                            if isRetail then
                                ...
                                if drFrame.Cooldown then
                                    drFrame.Cooldown:SetSwipeTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\talentsmasknodechoiceflyout")
                                end
                                drFrame.Mask:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\talentsmasknodechoiceflyout", "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
                            end
                            if drFrame.Icon then
                                drFrame.Icon:SetTexCoord(0.05, 0.95, 0.05, 0.9)
                                drFrame.Icon:AddMaskTexture(drFrame.Mask)
                ...
                for drIndex, fakeDRFrame in ipairs(frame.fakeDRFrames) do
                    if fakeDRFrame then
                        ...
                        elseif db.drBorderGlowOff then
                            ...
                            if fakeDRFrame.Cooldown then
                                fakeDRFrame.Cooldown:SetSwipeTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\squarecutcornermask")
                            end
                            fakeDRFrame.Mask:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\squarecutcornermask", "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
                            if fakeDRFrame.Icon then
                        ...
                        elseif db.brightDRBorder then
                            ...
                            if fakeDRFrame.Border then
                                fakeDRFrame.Border:Show()
                                fakeDRFrame.Border:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-HUD-ActionBar-PetAutoCast-Mask.tga")
                                if db.blackDRBorder then
                                    fakeDRFrame.Border:SetVertexColor(0, 0, 0, 1)
                                else
                                    if drIndex == 1 then
                            ...
                            if isRetail then
                                ...
                                if fakeDRFrame.Cooldown then
                                    fakeDRFrame.Cooldown:SetSwipeTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\talentsmasknodechoiceflyout")
                                end
                                fakeDRFrame.Mask:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\talentsmasknodechoiceflyout", "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
                            end
                            fakeDRFrame.Icon:SetTexCoord(0.05, 0.95, 0.05, 0.9)
                            fakeDRFrame.Icon:AddMaskTexture(fakeDRFrame.Mask)
    ...
    for i = 1, sArenaMixin.maxArenaOpponents do
        ...
        for n = 1, #categories do
            ...
            elseif db.drBorderGlowOff then
                ...
                dr.Border:SetTexture("Interface\\Buttons\\UI-Quickslot-Depress")
                dr.Cooldown:SetSwipeTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\squarecutcornermask")
                dr.Mask:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\squarecutcornermask", "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
            ...
            elseif db.brightDRBorder then
                ...
                if isRetail then
                    dr.Border:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-HUD-ActionBar-PetAutoCast-Mask.tga")
                    ...
                    dr.Border:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\UI-HUD-ActionBar-PetAutoCast-Mask.tga")
                    dr.Cooldown:SetSwipeTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\talentsmasknodechoiceflyout")
                    dr.Mask:SetTexture("Interface\\AddOns\\sArena_Updated2_by_sammers\\Textures\\talentsmasknodechoiceflyout", "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
                end
                dr.Icon:SetTexCoord(0.05, 0.95, 0.05, 0.9)
                ...
                args = {
                    ...
                    streamerProfilesDesc = {
                        ...
                        name = function(info)
                            ...
                            local currentProfileKey = sArena3DB.profileKeys[fullKey] or "Default"
