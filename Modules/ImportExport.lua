...
function sArenaMixin:ExportProfile(profileKeySupplied)
    ...
    local profileKey = profileKeySupplied or sArena3DB.profileKeys[fullKey]
    if not profileKey then
        return nil, L["Message_NoProfileFound"]
    end

    local profileTable = sArena3DB.profiles[profileKey]
...
function sArenaMixin:ImportProfile(encodedString, customProfileName, externalSource)
    ...
    sArena3DB.profiles[newName] = importTable.data

    -- Apply profile to current character
    for nameRealm in pairs(sArena3DB.profileKeys) do
        sArena3DB.profileKeys[nameRealm] = newName
    end
...
function sArenaMixin:ImportStreamerProfile(streamerName, profileString, displayName, classColor)
    ...
    local profileExists = sArena3DB.profiles[profileName] ~= nil

    -- Get current profile name
    local name, realm = UnitName("player")
    realm = realm or GetRealmName()
    local fullKey = name .. " - " .. realm
    local currentProfileKey = sArena3DB.profileKeys[fullKey]
